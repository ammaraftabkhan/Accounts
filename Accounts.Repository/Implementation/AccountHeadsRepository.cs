...
namespace Accounts.Repository.Implementation
{
    public class AccountHeadsRepository: IAccountHeadsRepository
    {
        ...
        {
            var getId = _AccuteDbContext.AccountHeads.Any() ? _AccuteDbContext.AccountHeads.Max(e => e.AcHeadTypeId) + 1 : 1;
        ...
        public bool DeleteAccountHead(int id)
        {
            if (id > 0)
            {
                ...
                try
                {
                    ...
                    if (data != null)
                    {
                        _AccuteDbContext.AccountHeads.Remove(data);
                        return _AccuteDbContext.SaveChanges() > 0;
                    }
                    return false;
        ...
        public AccountHead FindAccountHead(long id)
        {
            if (id > 0)
            {
                try
                {
                    ...
                    var data = _AccuteDbContext.AccountHeads.Find(id);
                    return data;
                }
                catch (Exception ex)
                {
        ...
        public List<AccountHead> GetAccountHead()
        {
            try
            {
                return _AccuteDbContext.AccountHeads.ToList();
            }
            catch
            {
